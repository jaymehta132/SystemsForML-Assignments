...
int main(int argc, char** argv){
    ...
    int subB = batchSize/4;
    cudaEvent_t start, stop;
    cudaEventCreate(&start);
    cudaEventCreate(&stop);

    dim3 threads(32, 32);
    dim3 blocks(((batchSize/4) + threads.x - 1)/threads.x, (N + threads.y - 1)/threads.y);

    cudaEventRecord(start);
    for (int i = 0; i < 4; i++){
        int offset = i * subB * N;
    
        matmul<<<blocks, threads, 0, streams[i]>>>(dX + offset, dW1, dZ1 + offset, subB, N, N);
        CUDA_CHECK(cudaGetLastError());
    
        relu<<<blocks, threads, 0, streams[i]>>>(dZ1 + offset, dY + offset, subB, N);
        CUDA_CHECK(cudaGetLastError());
    
        matmul<<<blocks, threads, 0, streams[i]>>>(dY + offset, dW2, dZ + offset, subB, N, N);
        CUDA_CHECK(cudaGetLastError());
    }
