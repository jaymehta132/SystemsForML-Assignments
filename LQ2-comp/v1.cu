                      ...
                      float* C_h)
{
    ...
    if (N % 128 == 0) {
        ...
        matmul_fast_kernel<<<grid, block, 0, stream>>>(A_d, B_d, C_d, N);
    } else {
        dim3 block(16, 16);
        dim3 grid((N + block.x - 1) / block.x,
                  (N + block.y - 1) / block.y);
        matmul_kernel_naive<<<grid, block, 0, stream>>>(A_d, B_d, C_d, N);
    }
...
int main()
{
    ...
    printf("=== Speed Tests ===\n");
    {
        const std::vector<int> small_sizes = {64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768};
        // const std::vector<int> small_sizes = {16384, 32768};
        ...
        for (int N : small_sizes) {
            ...
            printf("Running GPU matmul for N = %d...\n", N);
